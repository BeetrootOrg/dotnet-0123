...
namespace SimpleServer.Controllers
{
    ...
    public class PersonsController : ControllerBase
    {
        ...
        {
            return Ok(await _persons.People.ToArrayAsync(cancellationToken));
        }
        ...
        {
            Person person = await _persons.People.FirstOrDefaultAsync(x => x.Id == id, cancellationToken);
            return person == null
                ? NotFound(id)
                : Ok(person);
        }
        ...
        {
            int id = await _persons.People.AnyAsync(cancellationToken)
                ? await _persons.People.MaxAsync(x => x.Id, cancellationToken) + 1
                : 1;

            _ = await _persons.AddAsync(new Person
            {
                Age = request.Age,
                Dob = request.Dob,
                Email = request.Email,
            ...
            _ = await _persons.SaveChangesAsync(cancellationToken);
            ...
            CancellationToken cancellationToken)
        {
            ...
            foreach (System.Reflection.PropertyInfo propertyInfo in request.GetType().GetProperties())
            {
                object fieldValue = propertyInfo.GetValue(request);

                if (fieldValue != null)
                {
                    System.Reflection.PropertyInfo destinationProperty = personType.GetProperty(propertyInfo.Name);
