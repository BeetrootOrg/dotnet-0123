...
namespace ConsoleApp.Services
{
    ...
    internal class RedisCacheClient : ICacheClient
    {
        ...
        {
            await using MemoryStream serialized = await _serializer.Serialize(value, token);
            _ = await _database.StringSetAsync(key.ToKey(), serialized.ToArray(), _ttl);
        }

        public async Task<T> Get<T>(IKey key, CancellationToken token = default)
        {
            RedisValue redisValue = await _database.StringGetAsync(key.ToKey());
            ...
            await using MemoryStream stream = new(redisValue);
